...
const nodeFormSchema = z.object({
  ...
  status: z.enum(['Online', 'Offline', 'Error', 'Connecting']).default('Offline'), // Default status
});

type NodeFormData = Omit<ServerNode, 'id'>; // We handle ID generation on save

interface AddNodeDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: ServerNode) => void; // Expects the full ServerNode with ID
...
export function AddNodeDialog({ isOpen, onClose, onSave, nodeData }: AddNodeDialogProps) {
  ...
  const onSubmit = (data: NodeFormData) => {
    const dataToSave: ServerNode = {
      ...(nodeData || { id: "" }), // Keep existing ID if editing, or it will be replaced
      ...data,
    };
    onSave(dataToSave); // Parent component (ServerNodesTable) will handle ID generation for new nodes
  };
